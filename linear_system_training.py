...
# Linear System environment (paper §4.1)
class LinearSystemEnv(gymnasium.Env):
    def __init__(self):
        super().__init__()
        self.A = np.array([[0.8, -0.1], [0.1, 0.7]])
        ...
        # self.K = la.inv(self.R + self.B.T @ P @ self.B) @ (self.B.T @ P @ self.A)
        self.K, _, _ = dlqr(self.A, self.B, self.Q, self.R)
        self.step_count = 0
    ...
    def step(self, action):
        ...
        next_state = self.A @ self.state + self.B @ action.reshape(-1)
        next_state = next_state.astype(np.float32)
        cost = self.state.T @ self.Q @ self.state + action.T @ self.R @ action
        reward = -cost
        # done = np.any(np.abs(next_state) > 100.0)
...
sakc_model_dir = current_path + '/linear_system_models_sakc/'
skvi_model_dir = current_path + '/linear_system_models_skvi/'
sacv_model_dir = current_path + '/linear_system_models_sacv/'
sacq_model_dir = current_path + '/linear_system_models_sacq/'
reward_dir = current_path + '/linear_system_rewards/'
