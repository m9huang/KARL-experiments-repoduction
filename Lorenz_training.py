...
class LorenzEnv(gymnasium.Env):
    def __init__(self):
        ...
        self.state_dim = 3
        self.action_dim = 1
        self.max_action = 75.0
        self.observation_space = spaces.Box(
            low=np.array([-20, -50, 0], dtype=np.float32),
            high=np.array([20, 50, 50], dtype=np.float32)
        )
        ...
        self.x_e = np.array([np.sqrt(self.beta*(self.rho - 1)),
                            np.sqrt(self.beta*(self.rho - 1)),
                            self.rho - 1], dtype=np.float32)
        self.dt = 0.01
        self.Jacobian_x = np.array([[-self.sigma, self.sigma, 0],
                                    [self.rho - self.x_e[2], -1, -self.x_e[0]],
                                    [self.x_e[1], self.x_e[0], -self.beta]], dtype=np.float32)
        self.Jacobian_u = np.array([[1], [0], [0]], dtype=np.float32)
    ...
    def reset(self, seed=None, options=None):
        ...
        self.state = np.random.uniform(
            low=0.5 * self.observation_space.low,
            high=0.5 * self.observation_space.high,
            size=(self.state_dim,)
        ).astype(np.float32)
        self.step_count = 0
        return self.state, {}
    ...
    def continuous_dynamics(self, action):
        def dx_dt(t, state):
            f = np.array([self.sigma*(state[1] - state[0]) + action,
                          (self.rho - state[2])*state[0] - state[1],
                            state[0]*state[1] - self.beta*state[2]], dtype=np.float32)
            return f
        return dx_dt
    ...
    def step(self, action):
        ...
        # next_state = np.array([self.sigma*(self.state[1] - self.state[0]) + action_val,
        #                        (self.rho - self.state[2])*self.state[0] - self.state[1],
        #                        self.state[0]*self.state[1] - self.beta*self.state[2]], dtype=np.float32)
        next_state = self.f(self.state, action_val)
        cost = (self.state - self.x_e).T @ self.Q @ (self.state - self.x_e) + action.T @ self.R @ action
        reward = -cost
...
sakc_model_dir = current_path + '/Lorenz_models_sakc/'
skvi_model_dir = current_path + '/Lorenz_models_skvi/'
sacv_model_dir = current_path + '/Lorenz_models_sacv/'
sacq_model_dir = current_path + '/Lorenz_models_sacq/'
reward_dir = current_path + '/Lorenz_rewards/'
