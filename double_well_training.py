...
class DoubleWellEnv(gymnasium.Env):
    def __init__(self):
        ...
        self.state_dim = 2
        self.action_dim = 1
        self.Jacobian_x = np.array([[4, 0], [0, -2]], dtype=np.float32)
        self.Jacobian_u = np.array([[1.0], [1.0]], dtype=np.float32)
        self.max_state = 10.0
    ...
    def reset(self, seed=None, options=None):
        ...
        self.state = np.random.uniform(-2.0, 2.0, size=self.state_dim).astype(np.float32)
        self.step_count = 0
        return self.state, {}
    
    def continuous_dynamics(self, action):
        def dx_dt(t, state):
            # term = np.array([[0.7, state[0]], [0, 0.5]], dtype=np.float32) @ np.random.normal(0, 1, size=self.state_dim).astype(np.float32)
            f = np.array([4*state[0] - 4*state[0]**3 + action,
                           -2*state[1] + action], dtype=np.float32)
            return f
        return dx_dt
    
    def f(self, state, action):
        sigma_x = np.array([[0.7, state[0]], [0, 0.5]], dtype=np.float32)
        diffusion = sigma_x @ np.random.normal(0, 1, size=self.state_dim).astype(np.float32) * np.sqrt(self.dt)
        drift = self.continuous_dynamics(action)(0, state) * self.dt
    ...
    def step(self, action):
        ...
        # dx_dt = np.array([4*self.state[0] - 4*self.state[0]**3 + action_val,
        #                        -2*self.state[1] + action_val], dtype=np.float32) + term
        # dt = 0.01
        # next_state = self.state + dx_dt * dt
...
sacv_model_dir = current_path + '/double_well_models_sacv/'
sacq_model_dir = current_path + '/double_well_models_sacq/'
