...
class SAC_Q_Agent:
    def __init__(self, state_dim, action_dim, memo_capacity,
        ...
        self.target_critic_1 = CriticNetwork(beta, state_dim, action_dim, layer1_dim, layer2_dim).to(device)
        self.target_critic_2 = CriticNetwork(beta, state_dim, action_dim, layer1_dim, layer2_dim).to(device)
    ...
    def update(self):
        ...
        with torch.no_grad():
            next_action, next_log_prob = self.actor.sample_normal(next_state, reparameterize=False)
            target_q1_next = self.target_critic_1.forward(next_state, next_action)
            target_q2_next = self.target_critic_2.forward(next_state, next_action)
            target_q_next = torch.min(target_q1_next, target_q2_next) - self.alpha * next_log_prob
            target_q = reward + self.gamma * target_q_next * (1.0 - done.float())
